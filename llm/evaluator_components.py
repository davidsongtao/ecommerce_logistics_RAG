...
class QualityEvaluator(BaseEvaluator):
    ...
    def _evaluate_relevance(self, text: str, prompt: str) -> float:
        """评估文本与提示词的相关性"""
        try:
            return self.calculate_semantic_similarity(text, prompt)
        except Exception as e:
    ...
    def _evaluate_coverage(self, text: str, prompt: str) -> float:
        """评估文本对提示词关键信息的覆盖程度"""
        try:
            prompt_words = set(self.cut_words(prompt))
            text_words = set(self.cut_words(text))
    ...
    def _evaluate_consistency(self, text: str, prompt: str, context: str) -> float:
        ...
        try:
            ...
            return self.calculate_semantic_similarity(text, reference)
        except Exception as e:
